...
import com.dgq.utils.ServletUtils;
...
public class Struts2Utils {
	...
	/**
	 * HttpSession中Attribute的简化函数,放入session
	 */
	...
	public static void renderServlet(final String contentType,
			final String content, HttpServletResponse response,
			final String... headers) {
	...
	public static void renderXmlServlet(final String xml,
			HttpServletResponse response, final String... headers) {
		renderServlet(ServletUtils.XML_TYPE, xml, response, headers);
	}
	...
	private static HttpServletResponse initResponseHeaderServlet(
			final String contentType, HttpServletResponse response,
			final String... headers) {
		// 分析headers参数
		String encoding = DEFAULT_ENCODING;
		boolean noCache = DEFAULT_NOCACHE;
